          ...
          <div className={`absolute top-0 left-0 right-0 z-10 ${isMobile ? 'pt-[57px]' : ''}`}>
            <div className={`px-6 ${isMobile ? 'py-2' : 'py-6'}`}>
                  <div className={`flex items-center justify-center ${isMobile ? 'mb-2' : 'mb-4'} ${isMobile ? 'flex-row' : ''}`}>
                    <span className={`${isMobile ? 'text-3xl mr-1' : 'text-5xl mr-4'} ${showAnimations ? 'animate-bounce' : ''}`}>🌊</span>
                    ...
                    <span className={`${isMobile ? 'text-3xl ml-1' : 'text-5xl ml-4'} ${showAnimations ? 'animate-bounce' : ''}`}>🌊</span>
                  </div>
                  ...
                  <p className={`${isMobile ? 'text-base' : 'text-base'} text-white drop-shadow-md text-center ${isMobile ? 'mb-2' : 'mb-4'}`}>📍 Click any marker to see beach details</p>
                  <div className={`text-center ${isMobile ? 'space-y-2' : 'space-y-3'}`}>
                    ...
                    {isMobile && (
                      <div className="flex space-x-3">
                        ...
                        <button 
                          className={`bg-gradient-to-r from-blue-500 to-cyan-600 hover:from-blue-600 hover:to-cyan-700 text-white px-6 py-[14px] rounded-full text-base font-semibold shadow-lg transition-all duration-200 hover:shadow-xl hover:-translate-y-0.5 ${showAnimations ? 'animate-bounce spin-360' : ''} ml-[2px]`}
                          onClick={() => {
